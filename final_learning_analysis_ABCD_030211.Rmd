...
```
- No effect of group or interaction tested either by lm or lmer
```{r,echo=FALSE,warning=FALSE}
...
### plot ASL RT slope across the two groups
* mean RT slope
```{r,echo=FALSE}
...
```
- within VSL, ANCOVA to test group effect, no group effect
```{r,echo=FALSE}
...
```
- within ASL, significant group effect: DD<TYP
```{r,echo=FALSE}
...
### generalized linear effect modeling: testing the interaction between group and task
- main effect of task (visual > auditory); main effect of group (TYP > DD); 
```{r,echo=FALSE}
...
### ancova: testing the interaction between group and task
- marginal effect of task (Visual > Auditory)
```{r,echo=FALSE,warning=FALSE}
...
task_data<-d_all%>%select(kbit_ss_2,gort_ori_ss_2,ctopp_nonword_raw_2,ctopp_elision_raw_2,ctopp_blending_raw_2,wais_dsb_ss_2,slopeProp_On,slope_mt,slope_me,vis_slope_scale,aud_slope_scale,,vis_acc,aud_acc,quicksin_snr_loss_2)
