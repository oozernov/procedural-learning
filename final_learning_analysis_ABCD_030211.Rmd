...
adult_table<-d%>%ungroup%>%dplyr::select(PartID,Subgroup,Age,IQ,WID,WA,SWE,PDE,
                                       Vocabulary,ORI,Elision,ORI,Nonword,RAN_2Set,DigitsForward,DigitsBackward)
abcd_b<-na.omit(adult_table)
table(d$Subgroup,d$Sex)
at<-summary(tableby(Subgroup ~ ., data = adult_table,
                     control=tableby.control(numeric.stats="meansd", total=FALSE)),title = " Descriptives",text=TRUE,digits=2, digits.p=3)
write2word(at, "descriptives.doc", title="Descriptives")
abcd_b %>% cohens_d(Nonword~ DD, var.equal = TRUE)
...
ggplot(task_data_dys, aes(x = wrmt_wa_ss_2, y = aud_slope_scale)) +
  stat_smooth (
    method = "glm",
    formula = y ~ x,
    colour = "black",
    size = 1) +
  geom_point(aes(size = 3)) +
  labs(x = "Word Attack ", y = "ASL Slope") +
  theme(axis.title = element_text(family = "Trebuchet MS", size = 14))+ theme(panel.background = element_blank(),legend.position = "none")
```
