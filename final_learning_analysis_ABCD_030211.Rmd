...
adult_table<-d%>%ungroup%>%dplyr::select(Subgroup,Age,IQ,WID,WA,SWE,PDE,
...
lmerrp2 <- lmer(prop_on~Subgroup*trial +Age+Sex + (1+trial|PartID),data=rp2_age_gender_iq)
...
```{r,warning = FALSE}
summary(lm(time~Age+Sex+kbit_ss+Subgroup,data=me_1))
summary(lm(time~Age+Sex+kbit_ss+Subgroup,data=mt_1))
```
...
```{r, echo=FALSE,warning = FALSE,include=FALSE}
lmerm3<-lmer(error~Age+Sex+kbit_ss+Subgroup*trial+(1+trial|PartID),
        data=mt2_age_gender_iq)
lmerm4<-lmer(error~Age+Sex+kbit_ss+Subgroup*trial+(1|PartID),
        data=mt2_age_gender_iq)
anova(lmerm3,lmerm4)
summary(lmerm3)
```
...
d_3 <- d %>%
   select("PartID","Subgroup", "Age","Sex")
...
model1_cov <- lm(rt_col~Age+Sex+kbit_ss+reindex*Subgroup,data = fam_tsl_age_gender_iq)
...
tsl_rt_ancova=aov(rt_slope~Subgroup+Age+Sex+kbit_ss,data=subj_table_tsl_gender_iq)
...
model1_cov <- lm(rt_col~Age+Sex+kbit_ss+reindex*Subgroup,data = fam_vsl_age_gender_iq)
print(summary(model1_cov))
...
model_rt_cov <- lm(rt_col~Age+Sex+kbit_ss+task*reindex*Subgroup,data=all_fam_trials_age_gender_iq)
...
```{r,echo=FALSE,warning = FALSE}
all_sl_slope_ancova = aov(rt_slope_scale~Subgroup*task+Age+Sex+kbit_ss,data=all_subj_slope_age_gender_iq)
...
-  VSL, no group effect
```{r,echo=FALSE}
model_vslacc_lmer_cov <- glmer(corr~Age+Sex+Subgroup+(1|PartID)+(1|trial),data=vsl_accuracy_age_gender_iq,family="binomial", control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
```{r,echo=FALSE}
model_tslacc_lmer_cov <- glmer(corr~Age+Sex+Subgroup+(1|PartID)+(1|trial),data=tsl_accuracy_age_gender_iq,family="binomial",control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
model_acc <- aov(subj_corr~task*Subgroup+Age+Sex+kbit_ss,data=all_acc_table_age_gender_iq)
...
task_data<-d_all%>%ungroup%>%dplyr::select(IQ,WID,WA,Vocabulary,ORI,Elision,Nonword,RAN_2Set,DigitsForward,DigitsBackward,slopeProp_On,slope_mt,slope_me,vis_slope_scale,aud_slope_scale,aud_fam_rt,vis_fam_rt)

task_data_dys<-d_all_dys%>%ungroup%>%dplyr::select(IQ,WID,WA,Vocabulary,ORI,Elision,Nonword,RAN_2Set,DigitsForward,DigitsBackward,slopeProp_On,slope_mt,slope_me,vis_slope_scale,aud_slope_scale,aud_fam_rt,vis_fam_rt)

task_data_typ<-d_all_typ%>%ungroup%>%dplyr::select(IQ,WID,WA,Vocabulary,ORI,Elision,Nonword,RAN_2Set,DigitsForward,DigitsBackward,slopeProp_On,slope_mt,slope_me,vis_slope_scale,aud_slope_scale,aud_fam_rt,vis_fam_rt)
...
ggplot(task_data_dys, aes(x = WA, y = aud_slope_scale)) +
  stat_smooth (
    method = "glm",
    formula = y ~ x,
    colour = "black",
