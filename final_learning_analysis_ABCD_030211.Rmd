...
- borderline significant effect trial x subgroup (fasrer learning in Dys)
```{r, echo=FALSE,warning = FALSE}
...
model1_cov_scale <- lm(rt_col~background_age+background_sex+kbit_ss+reindex*Subgroup,data=fam_tsl_s_age_gender_iq)
print(summary(model1_cov_scale))
model2_cov <- lmer(rt_col~background_age+background_sex+kbit_ss+reindex*Subgroup+(1|PartID)+(0+reindex|PartID),data=fam_tsl_age_gender_iq)
print(summary(model2_cov))
model2_cov_scale <- lmer(rt_col~background_age+background_sex+kbit_ss+reindex*Subgroup+(1|PartID)+(0+reindex|PartID),data=fam_tsl_s_age_gender_iq)
print(summary(model2_cov_scale))
```
##Plot of TSL RT
...
print(summary(model1_cov))
model1_cov_scale <- lm(rt_col~background_age+background_sex+kbit_ss+reindex*Subgroup,data=fam_vsl_s_age_gender_iq)
print(summary(model1_cov_scale))
```
...
print(summary(model2_scale))
model2_cov <- lmer(rt_col~background_age+background_sex+kbit_ss+reindex*Subgroup+(1|PartID)+(0+reindex|PartID),data=fam_vsl_age_gender_iq)
print(summary(model2_cov))
model2_cov_scale <- lmer(rt_col~background_age+background_sex+kbit_ss+reindex*Subgroup+(1|PartID)+(0+reindex|PartID),data=fam_vsl_s_age_gender_iq)
print(summary(model2_cov_scale))
```
##Plot of VSL RT
### RT as the function of Target repetition
...
model_rt <- lm(rt_slope_scale~task*Subgroup+mean_rt,data=all_subj_slope_complete) # controlling for mean rt.
...
model_rt_cov <- lm(rt_slope_scale~background_age+background_sex+kbit_ss+task*Subgroup+mean_rt,data=all_subj_slope_age_gender_iq)
print(summary(model_rt_cov))
model_rt_lmer_cov <- lmer(rt_col~background_age+background_sex+kbit_ss+task*Subgroup+(1+task|PartID)+(1|reindex),data=all_fam_trials_age_gender_iq)
print(summary(model_rt_lmer))
```
### Mean RT sanity check
No significant differences in RT for either task.
```{r,echo=FALSE,warning = FALSE}
m6<-lm(aud_fam_rt~background_age+background_sex+kbit_ss+Subgroup, data=d_all,na.action = na.exclude)
anova(m6)

m7<-lm(vis_fam_rt~background_age+background_sex+kbit_ss+Subgroup, data=d_all,na.action = na.exclude)
anova(m7)
```
...
model_vslacc_lmer_cov <- glmer(corr~background_age+background_sex+kbit_ss+Subgroup+(1|PartID)+(1|trial),data=vsl_accuracy_age_gender_iq,family="binomial")
print(summary(model_vslacc_lmer_cov))

model_tslacc_lmer_cov <- glmer(corr~background_age+background_sex+kbit_ss+Subgroup+(1|PartID)+(1|trial),data=tsl_accuracy_age_gender_iq,family="binomial")
