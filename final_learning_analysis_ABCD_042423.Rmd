...
#### Create a df with rp and mt measures
```{r, echo=FALSE,warning = FALSE}
d_mt <- mt2%>%select(PartID, Subgroup,time, error) %>%
  group_by(PartID) %>% 
  dplyr::summarise(mt_sum_time = sum(time),mt_sum_err = sum(error))%>%ungroup()

d_pl<-merge(d_rp,d_mt,all = TRUE)
#write.csv(d_pl, "PL_all042923.csv")
```
...
- remove outliers who have hit rate lower than and equal to 0 (remaining participant: 14 DD and 18 TYP)
...
adult_table<-arhq_d_dd2%>%ungroup%>%dplyr::select(Subgroup.x,Sex,Age,RC,IQ,WID,WA,SWE,PDE,Vocabulary,
