...
m1<-lm(prop_on~Subgroup*trial,
        data=rp2) 
summary(m1) #sig effect of trial and significant interaction between trial and group: DD learn faster!
...
summary(m2)
#lsmeans(m1, list(pairwise ~ Subgroup|trial), adjust = "tukey") # ZQ notes: I am not sure what lsmeans does here, as you are no longer treating trial as a factor.
```

###linear mixed-effect modeling: treating trial as a continuous variable: marginal group differences (lmerrp2)
```{r, echo=FALSE,warning = FALSE}
library(lmerTest)
...
anova(lmerrp1,lmerrp2) #ZQ notes: actually I see model comparison suggesting that trial as a random slope makes a significant difference in the model fit.
summary(lmerrp2)
...
### ANCOVA on individual slope - marginal group effect
```{r, echo=FALSE,warning = FALSE}
m2<-lm(slopeProp_On~kbit_ss+Subgroup, data=d_rp) # ZQ notes: I added kbit_ss here because the two groups are not significantly different on age/sex, but only on IQ.
anova(m2)
...
multi.group3 <- 
  ...
  dabestr::dabest(Subgroup,mean, 
         idx = list(c("TYP",'DD')),
         paired = FALSE)
plot(multi.group3, palette=c("blue","red"),rawplot.ylabel = "RP Prop On Slope") # ZQ notes: on my computer, this plot does not work.
```

# Mirror Tracing (No Group Difference)
...
m3<-lm(time~Subgroup*trial+background_age+background_sex+kbit_ss,
        data=mt2_age_gender_iq) #after controlling for age, sex, and iq, the results remain the same: main effect of trial, but no effect of group
summary(m3)
m4<-lm(error~Subgroup*trial+background_age+background_sex+kbit_ss,
        data=mt2_age_gender_iq) #after controlling for age, sex, and iq, the results remain the same: main effect of trial, but no effect of group
summary(m4)
```
## linear mixed modeling: main effect of trial, no effect of subgroups
```{r, echo=FALSE,warning = FALSE}
...
#### growth curve analysis on completion time : no group effect
```{r, echo=FALSE}
...
```
#### growth curve analysis on error: no group effect
```{r, echo=FALSE}
...
d2<-merge(d_rp,d_mt) # ZQ notes: the two groups are significantly different on IQ
table(d2$Subgroup)
```

###MT: ANCOVA controlling for age, sex, and kbit on individual subject's Slope
```{r, echo=FALSE,warning = FALSE}
...
```
###Dys only: both the rotary pursuit and ASL accuracy/RT are related to reading

```{r, echo=FALSE}
corstars(task_data_dys,method="pearson")#you need to run the function first
```

###Typ only: better VSL is related to worse reading...
```{r, echo=FALSE}
