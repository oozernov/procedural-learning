...
```{r,echo=FALSE,warning=FALSE}
usable_tsl_rt<-subj_table_tsl[subj_table_tsl$hit_rate>0.25,]$list_tsl_id
subj_table_tsl_usable <-subset(subj_table_tsl,list_tsl_id %in% usable_tsl_rt)
...
```{r,echo=FALSE,warning=FALSE}
t.test(dprime~group,data=subj_table_tsl_usable)
t.test(rt_slope~group,data=subj_table_tsl_usable)
fam_trial_tsl_usable <-subset(fam_trial_tsl,id %in% usable_tsl_rt)
fam_trial_tsl_usable_s <-subset(fam_trial_tsl_scale,id %in% usable_tsl_rt)
model1 <- lm(rt_col~reindex*group_cond,data=fam_trial_tsl_usable)
print(summary(model1))
model1_scale <-lm(rt_col~reindex*group_cond,data=fam_trial_tsl_usable_s)
print(summary(model1_scale))
model2 <- lmer(rt_col~reindex*group_cond+(1|id)+(0+reindex|id),data=fam_trial_tsl_usable)
print(summary(model2))
model2_scale <- lmer(rt_col~reindex*group_cond+(1|id)+(0+reindex|id),data=fam_trial_tsl_usable_s)
print(summary(model2_scale))
```
...
### RT as the function of Target repetition
```{r,echo=FALSE}
group_rt_mean_tsl = cast(fam_trial_tsl,reindex+group_cond~.,value="rt_col",mean)
colnames(group_rt_mean_tsl)[3]="mean_RT"
group_rt_sd_tsl = cast(fam_trial_tsl,reindex+group_cond~.,value="rt_col",sd)
colnames(group_rt_sd_tsl)[3]="sd_RT"
group_rt_length_tsl = cast(fam_trial_tsl,reindex+group_cond~.,value="rt_col",length)
colnames(group_rt_length_tsl)[3]="length"
group_rt_tsl = merge(group_rt_mean_tsl,group_rt_sd_tsl)
group_rt_tsl = merge(group_rt_tsl,group_rt_length_tsl)
group_rt_tsl$se = group_rt_tsl$sd/sqrt(group_rt_tsl$length)
summary(group_rt_tsl)
pd <- position_dodge(0.2)
ggplot(group_rt_tsl,aes(x = reindex,y = mean_RT,colour = group_cond)) +
  theme_classic(base_size = 20.0) +
  xlab(label = 'Trial Index') +
  ylab(label = 'Response time during\n TSL Familiarization (ms)') +
  scale_x_continuous(breaks = pretty_breaks()) +
  scale_y_continuous(breaks = pretty_breaks(),limits=c(0,700)) +
  scale_colour_hue(name="Group",    # Legend label, use darker colors
                     breaks=c("DD", "TYP"),
                     labels=c("Dyslexia", "Typical"),
                     l=40) +
...
cbbPalette <- c("#000000","#000000")
ggplot() +
  geom_bar(aes(x = group,y = rt_slope,fill = group),data=subj_table_tsl_usable,colour = '#000000',fun.data = mean_sdl,fun.args = list(mult = 1),stat = 'summary',position = position_dodge(width = 0.9)) +
  theme_classic(base_size = 18.0) +
  ylab(label = 'Response Time Slope (ms/Trial)') +
  xlab(label = 'Group') +
  #coord_cartesian(ylim = c(0.3,1)) +
  geom_errorbar(aes(y = rt_slope, x = group),data=subj_table_tsl_usable,size = 0.3,width = 0.2,fun.y = function(x) mean(x),fun.ymin = function(x) mean(x) - sd(x)/sqrt(length(x)),fun.ymax = function(x) mean(x) + sd(x)/sqrt(length(x)) ,stat = 'summary')+
  geom_beeswarm(aes(x = group,y = rt_slope, colour = group),data=subj_table_tsl_usable,dodge.width=0.9,cex=2.5) +
  scale_fill_brewer(palette = 'Set1') + 
  scale_color_manual(values=cbbPalette)
```
...
```{r,echo=FALSE}
ggplot() +
  geom_bar(aes(x = group,y = rt_slope_scale,fill = group),data=subj_table_tsl_usable,colour = '#000000',fun.data = mean_sdl,fun.args = list(mult = 1),stat = 'summary',position = position_dodge(width = 0.9)) +
  theme_classic(base_size = 18.0) +
  ylab(label = 'Scaled Response Time Slope\n(au/trial)') +
  xlab(label = 'Group') +
  #coord_cartesian(ylim = c(0.3,1)) +
  geom_errorbar(aes(y = rt_slope_scale, x = group),data=subj_table_tsl_usable,size = 0.3,width = 0.2,fun.y = function(x) mean(x),fun.ymin = function(x) mean(x) - sd(x)/sqrt(length(x)),fun.ymax = function(x) mean(x) + sd(x)/sqrt(length(x)) ,stat = 'summary')+
  geom_beeswarm(aes(x = group,y = rt_slope_scale, colour = group),data=subj_table_tsl_usable,dodge.width=0.9,cex=2.5) +
  scale_fill_brewer(palette = 'Set1') + 
  scale_color_manual(values=cbbPalette)
```
...
model_rt <- aov(rt_slope_scale~task*group+mean_rt+Error(subj/task),data=all_subj_slope_complete) # controlling for mean rt.
print(summary(model_rt))
model_rt_lmer <- lmer(rt_col~task*group_cond+(1+task|id)+(1|reindex),data=all_fam_trials)
...
stat.test <- all_subj_slope %>%
  group_by(task) %>%
  tukey_hsd(rt_slope_scale~group)
stat.test
ggplot() +
  geom_bar(aes(x = group,y = rt_slope_scale,fill = group),data=all_subj_slope,colour = '#000000',fun.data = mean_sdl,fun.args = list(mult = 1),stat = 'summary',position = position_dodge(width = 0.9)) +
  theme_classic(base_size = 18.0) +
  ylab(label = 'Scaled Response Time Slope\n(au/trial)') +
  xlab(label = 'Group') +
  #coord_cartesian(ylim = c(0.3,1)) +
  geom_errorbar(aes(y = rt_slope_scale, x = group),data=all_subj_slope,size = 0.3,width = 0.2,fun.y = function(x) mean(x),fun.ymin = function(x) mean(x) - sd(x)/sqrt(length(x)),fun.ymax = function(x) mean(x) + sd(x)/sqrt(length(x)) ,stat = 'summary')+
  geom_beeswarm(aes(x = group,y = rt_slope_scale, colour = group),data=all_subj_slope,dodge.width=0.9,cex=2.5) +
  facet_wrap(task~.) +
  scale_fill_brewer(palette = 'Set1') + 
  scale_color_manual(values=cbbPalette)
```
...
vsl_acc_table$group<-""
vsl_acc_table[vsl_acc_table$list_vsl_id %in% DD,]$group = "Dyslexic"
vsl_acc_table[vsl_acc_table$list_vsl_id %in% TYP,]$group = "Typical"
colnames(vsl_acc_table)[1] = "subj"
...
tsl_acc_table$group<-""
tsl_acc_table[tsl_acc_table$list_tsl_id %in% DD,]$group = "Dyslexic"
tsl_acc_table[tsl_acc_table$list_tsl_id %in% TYP,]$group = "Typical"
colnames(tsl_acc_table)[1] = "subj"
...
for(id in DD){
  DD_acc_vsl<- append(DD_acc_vsl,vsl_acc_table$subj_corr[vsl_acc_table$subj==id])
  DD_acc_tsl<- append(DD_acc_tsl,tsl_acc_table$subj_corr[tsl_acc_table$subj==id])
}

for(id in TYP){
  TYP_acc_vsl<- append(TYP_acc_vsl,vsl_acc_table$subj_corr[vsl_acc_table$subj==id])
  TYP_acc_tsl<- append(TYP_acc_tsl,tsl_acc_table$subj_corr[tsl_acc_table$subj==id])
}
...
```{r,echo=FALSE,warning=FALSE}
all_acc_table_complete = subset(all_acc_table,subj!="ABCD_1708")
all_acc_table_complete = subset(all_acc_table_complete,subj!="ABCD_1727")
model_acc <- aov(subj_corr~task*group+Error(subj/task),data=all_acc_table_complete)
...
```{r,echo=FALSE}
model_acc_lmer <- glmer(corr~task*group+(1+task|subj)+(1|trial),data=all_accuracy,family="binomial")
...
stat.test <- all_acc_table %>%
  group_by(task) %>%
  tukey_hsd(subj_corr~group)
stat.test
stat.test <- stat.test %>% add_xy_position(x = "group", fun = "mean_se")
all_acc_table$group = as.factor(all_acc_table$group)
ggbarplot(all_acc_table, x = "group", y = "subj_corr", fill = 
            "group", add = "mean_se", facet.by = c("task")) +
...
```{r,echo=FALSE}
ggplot() +
  geom_bar(aes(x = group,y = subj_corr,fill = group),data=all_acc_table,colour = '#000000',fun.data = mean_sdl,fun.args = list(mult = 1),stat = 'summary',position = position_dodge(width = 0.9)) +
  theme_classic(base_size = 18.0) +
  ylab(label = '% Correct') +
  xlab(label = 'Group') +
  #coord_cartesian(ylim = c(0.3,1)) +
  geom_errorbar(aes(y = subj_corr, x = group),data=all_acc_table,size = 0.3,width = 0.2,fun.y = function(x) mean(x),fun.ymin = function(x) mean(x) - sd(x)/sqrt(length(x)),fun.ymax = function(x) mean(x) + sd(x)/sqrt(length(x)) ,stat = 'summary')+
  geom_beeswarm(aes(x = group,y = subj_corr, colour = group),data=all_acc_table,dodge.width=0.9,cex=2.5) +
  facet_wrap(task~.) +
  scale_fill_brewer(palette = 'Set1') + 
  scale_color_manual(values=cbbPalette)
```
