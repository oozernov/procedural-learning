...
```
* but the group difference is marginal when tested by multi-level model with random intercept and random slope on reindex.
```{r,echo=FALSE,warning=FALSE}
library(lmerTest)
...
ggplot() +
  ...
  geom_point(aes(x = reindex,y = rt_col,colour = group_cond),data=fam_trial_vsl,shape = 23, size = 3) +
  geom_smooth(aes(x = reindex,y = rt_col,colour = group_cond),data=fam_trial_vsl,method = 'lm',formula = 'y ~ x',se = FALSE)
```
###Boxplot of the first half of the trials is significantly greater than the latter half of the trial
```{r,echo=FALSE}
par(mfrow=c(1,2))
boxplot(fam_trial_vsl$rt_col[fam_trial_vsl$reindex<total_vsl_trial/2], main="RT - first half of the trials",col="deepskyblue4",ylim=c(-400,800))
boxplot(fam_trial_vsl$rt_col[fam_trial_vsl$reindex>total_vsl_trial/2], main="RT - latter half of the trials",col="deepskyblue4",ylim=c(-400,800))
```
#Compute RT for TSL
